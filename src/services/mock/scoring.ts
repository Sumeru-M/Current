...
/**
 * Scoring factors — the pure core of the ranking engine.
 *
 * Extracted from `ranking.ts` so each factor can be tested in isolation, which
 * is where the ranking bugs lived: vibe position, budget fit, non-monotonic
 * occupancy. `ranking.ts` keeps the composition (`explainMatch`) and travel
 * estimation; this file holds the maths.
 *
 * `scoreFreshness` takes an injectable `at` clock, following `lib/hours.ts`, so
 * `explainMatch` can be made fully deterministic in a test.
 */
...
export const REFERENCE_POINT = { lat: 12.9716, lng: 77.5946 };
...
export const bandSpend: Record<Venue["priceBand"], number> = {
  1: 60000,
  2: 120000,
  3: 260000,
  4: 500000,
 ...
 */
export const vibeAffinity = (venue: Venue, vibe: string): number => {
...
export const scoreVibe = (intent: Intent, venue: Venue): MatchFactor => {
...
export const scoreQueue = (intent: Intent, live: Availability): MatchFactor => {
 ...
 */
export const scoreCapacity = (live: Availability): MatchFactor => {
...
export const scoreGroup = (
  ...
  const fits = offers
    .filter((o) => o.remaining > 0 && size >= o.minGuests && size <= o.maxGuests)
...
/** Clock is injectable so `explainMatch` can be made deterministic in a test. */
export const scoreFreshness = (
  live: Availability,
  at: Date = new Date(),
): MatchFactor => {
  const ageMin = (at.getTime() - new Date(live.updatedAt).getTime()) / 60_000;
  if (ageMin <= 15)
    return factor(
      "freshness",
      "Data",
...
export const scoreQuality = (venue: Venue): MatchFactor =>
  factor(
    "quality",
    "Reputation",
...
export const applyRequirements = (
  ...
  for (const requirement of intent.requirements) {
    ...
    if (key === "no cover") {
      ...
      out.push(factor(`req_${key}`, requirement, 5, `${requirement} available`));
    }
  }
  return out;
};
