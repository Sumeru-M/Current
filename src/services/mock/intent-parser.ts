...
import type { Intent, IntentTurn } from "@/types/intent";

/**
 * Intent parsing — the pure core of the mock engine.
 *
 * Extracted from `intent-engine.ts` so it can be tested without the async
 * latency, `Math.random()` id, and clock that the service wraps around it.
 * Everything here is a pure function of its input, following the same
 * pure/impure split already used by `lib/hours.ts` and `config/*`.
 *
 * In production the LLM replaces these regexes, but the *contract* — free text
 * in, a validated `Intent` out — is exactly what these functions pin down.
 */
...
export const VIBE_LEXICON: { match: RegExp; vibe: string }[] = [
...
export const REQUIREMENT_LEXICON: { match: RegExp; requirement: string }[] = [
...
export const parseGroupSize = (raw: string): number | null => {
  ...
  if (weAre) {
    const token = weAre[1].toLowerCase();
    const n = Number(token);
    if (!Number.isNaN(n)) return clampGroup(n);
    if (NUMBER_WORDS[token]) return clampGroup(NUMBER_WORDS[token]);
  }
...
export const parseBudget = (
  ...
  const bare = text.match(/\b(\d{3,6})\s*(?:budget|each|per head|a head|pp)\b/i);
...
export const parseTiming = (text: string): Intent["timing"] => {
  if (/\bright now\b|\bnow\b|\basap\b|\bimmediately\b/i.test(text)) return "now";
...
export const parseQueue = (text: string): number | null => {
  const explicit = text.match(
...
export const parseTravel = (text: string): number | null => {
  const explicit = text.match(
...
export const parseNeighbourhood = (text: string): string | null =>
  NEIGHBOURHOODS.find((n) =>
    new RegExp(n.replace(/\s/g, "\\s?"), "i").test(text),
  ) ?? null;

export const parseVibes = (text: string): string[] =>
  parseList(
    text,
    VIBE_LEXICON.map((v) => ({ match: v.match, value: v.vibe })),
  );

export const parseRequirements = (text: string): string[] =>
  parseList(
    text,
    REQUIREMENT_LEXICON.map((r) => ({ match: r.match, value: r.requirement })),
  );
 ...
 */
export const scoreConfidence = (intent: Intent): number => {
...
export const buildUnderstanding = (intent: Intent): string => {
  const parts: string[] = [];
  if (intent.groupSize) {
    parts.push(
...
export const buildClarification = (
  ...
  if (!intent.vibes.length) {
    return {
      ...
      options: [
        { label: "Dancefloor", patch: { vibes: ["Dancefloor", "High energy"] } },
...
/**
 * The whole slot-filling pipeline as one pure function: free text (+ any prior
 * intent to refine) → a fully-populated draft with confidence scored. The
 * service wraps this with latency, an id, and a timestamp; tests call it
 * directly.
 */
export const parseIntent = (
  ...
  const draft: Intent = {
    ...
    vibes: mergeUnique(base.vibes, parseVibes(text)),
    requirements: mergeUnique(base.requirements, parseRequirements(text)),
    neighbourhood: parseNeighbourhood(text) ?? base.neighbourhood,
  ...
  return { ...draft, confidence: scoreConfidence(draft) };
};
